...
fn solve_part_1(input: &str) {
    ...
    for (i, g1) in glxs.iter().enumerate() {
        let ex_l_1 = empty_lines.iter().take_while(|l| **l < g1.0).count() as i64;
        let ex_c_1 = empty_cols.iter().take_while(|c| **c < g1.1).count() as i64;
        for g2 in glxs.iter().skip(i + 1) {
            let ex_l_2 = empty_lines.iter().take_while(|l| **l < g2.0).count() as i64;
            let ex_c_2 = empty_cols.iter().take_while(|c| **c < g2.1).count() as i64;
            let dist =
